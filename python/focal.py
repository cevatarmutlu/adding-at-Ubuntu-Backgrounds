...
class Focal(CustomXML):
    def __init__(self, xml_path, wallpapers_path, static_duration=1795, transition_duration=5):
    ...
    def generateFocalTags(self):
        focal_tags = [(self.create_element(self.root.makeelement("static", {}), self.static_key_value, {"file": self.wallpapers_path + "/" + image}), self.create_element(self.root.makeelement("transition", {}), self.transition_key_value, {"from": self.wallpapers_path + "/" + image, "to": self.wallpapers_list[i + 1] if i != len(self.wallpapers_list) - 1 else "/usr/share/backgrounds/Focal-Fossa_WP_4096x2304_GREY.png"})) for i, image in enumerate(self.wallpapers_list)]
        return focal_tags
